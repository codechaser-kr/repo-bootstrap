...
Codex와 Claude에서 공통으로 사용할 수 있는 레포 초기 세팅용 로컬 툴킷입니다.
...
- git-hooks: 타겟 저장소의 언어와 모노레포 구조를 분석해 Git hook 생성
...
레포를 직접 clone한 뒤 `./install.sh`를 실행하면 `branch`, `commit`, `pr`, `git-hooks`는 현재 체크아웃된 로컬 파일을 기준으로 설치됩니다. 아래 `curl` 또는 `wget` 예시는 GitHub `main` 브랜치를 기준으로 설치합니다.
