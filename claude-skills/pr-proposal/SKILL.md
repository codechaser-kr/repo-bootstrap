...
- Related issue lookup: 명시적 이슈 번호가 없을 때만 `gh issue list --limit 50 2>/dev/null || true`로 열린 이슈를 조회합니다.
...
5. 연관 이슈를 다음 순서로 확인합니다.
   ...
   - 명시적 번호가 없으면 `gh issue list --limit 50 2>/dev/null || true`로 열린 이슈를 조회하고, 이 PR의 주제(제목·변경 파일·diff)와 대조해 같은 기능/정책/버그를 다루는 이슈가 있는지 판단합니다. `gh`가 없거나 인증되지 않아 조회에 실패하면 브랜치명·커밋·diff 기반 판단만 계속하고 연관 이슈는 `- N/A`로 둡니다. 제목만으로 모호하면 `gh issue view <번호>`로 본문을 확인하되, 명령 인자에는 `#`을 제외한 숫자만 입력합니다(예: `gh issue view 123`).
