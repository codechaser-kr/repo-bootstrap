...
1. 목적과 배경을 먼저 설명합니다.
2. 브랜치명, 커밋 메시지, diff에서 `#123`, `issue-123`, `issues/123`, `GH-123` 같은 연관 이슈 번호를 확인합니다.
3. 연관 이슈가 명확하면 `Related: #123` 형식으로 적고, 이 PR이 이슈를 닫아야 한다는 명시가 있을 때만 `Closes #123` 형식을 사용합니다.
4. 연관 이슈가 없거나 확실하지 않으면 `- N/A`를 유지합니다.
5. 주요 변경사항은 짧은 불릿으로 정리합니다.
6. 영향 범위나 검토 포인트가 있으면 분리해서 적습니다.
7. 머지 전 확인사항은 체크리스트로 적습니다.
