...
5. 브랜치명, 커밋 메시지, diff에서 `#123`, `issue-123`, `issues/123`, `GH-123` 같은 연관 이슈 번호를 확인합니다.
6. 연관 이슈가 명확하면 `Related: #이슈번호` 형식으로 적고, 이 PR이 이슈를 닫아야 한다는 명시가 있을 때만 `Closes #이슈번호` 형식을 사용합니다.
7. 연관 이슈가 없거나 확실하지 않으면 `- N/A`를 유지합니다.
